...
- **スライドの実装**: 中身は **CSS transform**(`translateX`)でスライド（GPUで滑らか）。同時に AHK が**ウィンドウの可視リージョン**(`SetWindowRgn`)を**同じ時間配分(200ms linear)で同期クリップ**し、中身が退避した白いウィンドウ領域を隠す → 背景ごと右へスライドして見える。**ウィンドウは動かさない**ので Chrome の再描画が走らずカクつかない（位置スライドはカクつき＆2枚目モニターはみ出し、幅ドロワーはChrome最小幅制限、カラーキー透過はChromeが無視…で全て不可だったため、この方式に到達）。AHKは `timeBeginPeriod(1)` でSleep精度を上げ均一化。速度は `hotkey.ahk` の `DUR` と index.html の `transition` を一致させて調整
