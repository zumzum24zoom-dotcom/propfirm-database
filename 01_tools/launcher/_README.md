...
- **トグル開閉（右からスライド）**: `hotkey.ahk` が **Insert** で開閉。閉じる時はウィンドウを破棄せず隠すだけなので**ブラウザを起動し直さず**（ページも再読込されず履歴等を保持）、次が速い。窓の有無は EnumWindows でタイトル `PFD Launcher` を検索 → 多重起動しない
- **スライドの実装**: AHK は `WinShow`/`WinHide` で出し入れするだけ（エラーの出ない最小構成＝ホットキーが確実に効く）。スライドは**ページ側の CSS transform**(`translateX`, GPUで滑らか)。`visibilitychange` で、隠れた時は中身を画面外へ退避（無アニメ）、表示時に右からスライドインさせる。速度は index.html の `transition`(既定0.2s)で調整
  - **既知の制約**: しまう時は即時（ウィンドウ非表示が一瞬なのでスライドアウトは出せない）。また「白いウィンドウ枠ごとスライド」は Chrome の制約で不可（位置スライド=カクつき＆右の2枚目モニターはみ出し／幅ドロワー=Chrome最小幅／カラーキー透過=Chromeが無視）。中身(カード)のスライドインのみ滑らかに行う
- **ホットキーが効かない時**: 実験中に AHK のエラーダイアログが残って前面に居座ると入力を食う。`AutoHotkey64` を全終了して `hotkey.ahk` を起動し直す（管理者権限のアプリ前面時は、非管理者AHKがキーを取れない点にも注意）
- panel.ps1（ELECOMボタン用）は同じ非表示/再表示で動くが**スライドは無し（即時）**。スライドは Insert（AHK）経路のみ
- 窓を隠しても**サーバは常駐**（軽い）
