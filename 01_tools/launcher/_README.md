...
- **スライドの実装**: ウィンドウの**可視リージョン**(`SetWindowRgn`)を右端から伸縮させる。**背景パネルごと**右から開く/閉じる。ウィンドウを動かさず幅も変えないので、**右の2枚目モニターにはみ出さず**、Chromeの**最小ウィンドウ幅**制限にも当たらない（位置スライドや幅ドロワーは不可なため、この方式を採用）。速度は `hotkey.ahk` の `STEPS`/`STEP_MS` で調整
