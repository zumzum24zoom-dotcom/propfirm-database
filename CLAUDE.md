...
【複雑タスクの確認ルール】
- 複数ファイルの大幅変更・設計判断・初めて触る領域など、複雑になりそうな作業は着手前にマスターに確認を取る
- 確認形式：「このタスクは複雑です。トークン消費を度外視して全力で取り組みますか？ (y/n)」
- y → 拡張思考・高effort で全力実行
- n → 通常モードで実行
...
セッション起動時、上記の役割・指示形式を理解した後、以下を順に実行する：

1. `02_docs/HANDOFF.md` を必ず読み込む（進捗・タスク・設計判断の正本）
2. マスターへ以下を問う：
