...
- **名称**: Prop Firm Challengers（略称: PFD / PFC）
...
- **Page Maker**（`01_tools/page-maker-v12.html`）: 入力ツール。スロット格納・整形・Hugo出力
- **Hugo + Netlify**: 公開サイト
...
| ファイル | 用途 |
|---|---|
| `page-maker-v12.html` | データ入力・整形・Hugo出力エンジン |
| `sitemap-designer.html` | サイト構造（ナビゲーション）設計ツール |
| `widget-maker.html` | ウィジェット生成（試作） |
