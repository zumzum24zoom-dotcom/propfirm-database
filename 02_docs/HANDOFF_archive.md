# 引き継ぎ履歴アーカイブ — Prop Firm Challengers / propfirm-database

> **このファイルについて**: `HANDOFF.md`から移動した過去セッションの詳細記録・旧フェーズの仕様/タスクリスト。
> **通常は読まなくてよい**。「なぜこう決まったか」を調査するときだけ参照する。
> 新しいセッションログはここに追記せず、`HANDOFF.md`の「現在地」を更新したうえで、必要な詳細のみ本ファイル末尾に追記する。

---

## 【セッション13・2026-06-11】01_tools/utils フォルダ整理

### 削除したファイル

| ファイル | 理由 |
|---------|------|
| `BLT/site structre scanner` | SITE PRICEscannerの旧版(v1.0)・末尾に構文エラー(`s`)あり |
| `BLT/Scan price` | SITE PRICEscannerとバイト単位で完全重複 |
| `check-braces.js` | 旧page-maker-v11.html(行番号ハードコード)参照のデバッグ残骸。v12移行で無効 |
| `BLT/section-extractor-builder.html` | ブックマークレット生成UI（toc-scanner出力を消費するペアツール）。マスター判断で削除 |
| `url-picker.html` | ブックマークレット生成UI（独立ツール）。マスター判断で削除 |

### 移動・改名したファイル

| 旧 | 新 | 内容 |
|---|---|---|
| `site-scanner.js` | `BLT/price-scanner-v2-draft.js` | 価格要素検出+localStorage複数ページ蓄積+送信前プレビュー版。**未検証WIP**（HANDOFF次アクション「ブックマークレット動作確認」の対象） |
| `BLT/SITE PRICEscanner` | `BLT/price-scanner.js` | 現行稼働中の構造/価格スキャナー(v1.1) |
...
│   ├── toc-scanner.txt
│   ├── price-scanner.js          (現行)
│   └── price-scanner-v2-draft.js (未検証WIP)
├── html文章掃除.html
└── draft-viewer.html
```

### ドキュメント更新
- `PROJECT_HUB_v2.canvas`: site-scannerソースリンクを`BLT/price-scanner.js`に修正
- `02_docs/CLAUDE_CONTEXT.md`: utils/説明をリネーム後の構成に更新
...
| ファイル | 内容 |
|---------|------|
| `start-server.bat`（新） | vault root から port 8080 で HTTP サーバー起動。加工ツール全体の共通ランチャー |
| `PROJECT_HUB_v2.canvas` | 加工ツール4本のリンクを `localhost:8080/...` に変更。🚀 サーバー起動ボタン追加 |
...
# 旧フェーズ仕様・タスクリスト（〜セッション9・2026-06-09時点）

> 以下はPage Maker v11/Widget Maker期の仕様・進捗・タスクリスト。セッション10以降のデータ収集パイプライン路線とは焦点が異なる。
> 再着手する場合は内容の有効性を要確認。
...
## 5. サイト照合タスク（移設・転用リストをHugo実装に落とし込み・セッション9時点）
...
## 6. 移設タスク（旧Wraptas/Worker → Hugo・セッション9時点）
...
## 7. 転用タスク（考え方・アプローチの流用・セッション9時点）

> 転用 = 旧環境のロジック・設計思想を**新しい文脈に応用**する。DB_100_Impl 状態=転用 より自動取得。

- [ ] 縦タブプロンプトボタン修正（DBP_01/DBP_02）— Page Maker の UI設計をHugo側スロット構造に転用
- [x] 最低取引日数 表記統一 ver.01 — C系「最低取引日数（合格）」/F系「最低取引日数（出金）」→ MASTER_DEFS で正本化済み
- [x] 統制語彙統一・P1-P3クリーンアップ ver.01 — 不在表記ルール → `GLOSSARY_BASE` の notation に転用済み
- [ ] 汎用デザインエディタ（スタンドアロンHTMLツール）→ Page Maker / Widget Maker のテーマ設計に転用
- [x] パーマリンク実装（slugプロパティ追加）→ Hugo の slug ルーティング設計に転用済み
...
## 9. 【保留】Obsidian 本格運用構想（セッション9時点）

> Notion を Page Maker（構造化データ）と Obsidian（非構造ナレッジ）の2軸で完全代替する構想。
> セッション10以降はデータ収集パイプライン（scan/help-index/normalize-firm）路線に進んでおり、本構想は未着手のまま保留。
...
## セッション16（2026-06-13）：Web2MD導入 / v1廃止 / v2始動

### Web2MD MCP 導入
- `.mcp.json` に `web2md` サーバー登録（`npx -p web2md-mcp-server web2md-mcp`、APIキー埋込）
- Chrome拡張 `ijmgpkkfgpijifldbjafjiapehppcbcn` + Native Messaging Host `%LOCALAPPDATA%\Google\Chrome\User Data\NativeMessagingHosts\com.web2md.agent.json`
- 利用可能MCPツール: `convert_url` / `agent_convert` / `bridge_convert_url` / `agent_batch_convert` / `get_conversion` / `semantic_search`
- 検証: `convert_url`(サーバー側fetch) → fundingpips で403。`agent_convert`(Chrome拡張経由) → port 12315 起動条件あり。**実用は Chrome拡張ポップアップ「Download」手動運用**で確定
- Web2MD APIキー: `w2m_4a5f...15fa`（`.mcp.json` env直書き、`.gitignore`対象）

### v1廃止・削除（107ファイル + 2スキル）
- `data/firms/*.pipe.md` 31本（旧 normalize-firm 生成。master判定「ゴミ」）
- `data/firms/*.json.draft` 3本 / `data/plans/*.json.draft` 7本
- `data/scans/*.json` 33本（DOM Scanner出力）
- `data/help-index/*.json` 33本（Help Centerインデックス）
- `.claude/skills/normalize-firm/` / `.claude/skills/enumerate-help-index/`
- 保護: `data/firms/*.json` 34本 / `data/plans/*.json` 28本（Page Maker取込時に v2 で上書きされる設計のため残置）

### v2 パイプライン始動
- ディレクトリ: `data/firms-v2/{slug}.md`（拡張子 `.md`、v1と完全分離）
- 入力: `data/scans/{slug}/*.md`（Web2MD取得の記事単位）
- 整理: `scripts/split-web2md-dump.mjs`（Web2MD連続ダンプを記事ごと分割、ドメイン不一致は `_excluded_` プレフィクス）

### fundingpips（v2初号機完了）
- `data/scans/fundingpips/` 17ファイル（13本文 + 1誤混入隔離 + 3その他）
- 取得済: homepage / trading-objectives(4タブ) / 2-step-standard / prime / rewards / terms / prime-terms / tradin / Help記事5本(2-Step Standard, 2-Step Pro, 1-Step, Zero, 1K Instant)
- 404: Trading Rules / FAQ / Payouts Help（URL構造変更 → 他ファイルで代替済）
- 出力: `data/firms-v2/fundingpips.md`（F01-F22中18項目埋め + 5プラン比較テーブル + 価格表構造）
- 未確定: F03創業年、F06日本対応、F02法人住所詳細、価格セルの値

### セッション16 設計判断追加
- #8: v2 ファイル拡張子は `.md`（`.pipe.md` ではない）
- #9: Web2MD APIキーは `.mcp.json` env に直書き（`.env` ではなくMCPサーバー設定として保持）
- 既存「BLOCKED 5社」表記は誤り。実際は fundingpips 1社のみ完全ブロック。Web2MD Agent Bridgeで突破可能と実証

---

## 【セッション17-18・2026-06-14】Firm Database起点化・data整理・収集コックピット

- **18a**: `firm-database.html`（Firm Dashboard）を全作業の起点・管制塔に確定（設計判断#10）。旧 firm-dashboard/firm-urls/firm-tour を core から削除。tools.json は firm-database + NotebookLM のみ。
- **18b**: `data/`公開層 / `_work/`作業層 を物理分離（設計判断#12）。`firms-edit`/`firms-v2`/`scans`/`price-collect`/`help-index`/`pfdb.json`/`progress.json`/`firm-slot-urls.json` を `_work/` へ git mv。`_work/scans/` gitignore＋追跡解除（設計判断#5解消）。パス書換: agent.py/各script/normalize-firm-v2スキル。
- **18c**: v1ゴミ一掃。`pfdb.json`・`firms-edit/*.json`(33)・`help-index`(頓挫)・`_legacy/firm-urls.json`・`scripts/split-pfdb-to-edit.mjs` 削除（中身抽出済・損失なし・git履歴）。`data/firms`(34)/`data/plans`(28) は worklist 保持。
- **18d**: 収集コックピット FD-11〜15 実装。`_work/firm-slot-urls.json`(32社・URL↔スロット)起点。FD-12 URL生死(/api/url-health・urllib逐次・url-health.jsonキャッシュ)、FD-13 Firm網羅(F01-F22)、FD-14 NotebookLM導線(📓)、FD-15 Web2MD補完(🌐)。収集モデル: 主=NotebookLM＋公式URL保管/補完=Web2MD → Page Maker取込 → data/firms+plans。
- **18e**: Plan網羅率 FD-16＋help-index復活。観測: プランルール(P01-P19)は99%がHelp Center/Q&A＋利用規約。`_work/help-index/{slug}.json`(記事→P-スロット地図)。fundingpips を Zendesk API実取得→21/21。P-スロット宇宙=21項目(P20,P01,P02,P02b,P03-P19)。
- **18f-g**: help-index 横展開17社。Zendesk=fundingpips(API)、Intercom=13社(sitemap/巡回)、インラインFAQ/KB=4社(audacity/maven/hantec/moneta Playwright/curl)。`scripts/harvest-help-index.py`（機械タグ・要レビュー）。平均13.6/21。slug是正: alpha-capital→alpha-capital-group, maven→maven-trading（firm-slot-urls/help-index）。
- **18h**: FD-17 NotebookLM適性判定（check_urls→{code,textLen}・✅/⚠️/❌・閾値1200字）。bem-funding(独自FAQ・18社目)。一覧にF/P充填バー、パネルに✕閉じる。
