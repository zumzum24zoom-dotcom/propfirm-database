...
- 02_docs/HANDOFF.md を更新したが100行を超えて肥大化（現在地のみ保持・過去ログはarchiveへ）
...
def main():
    ...
    # ─── HANDOFF.md 肥大化チェック ───
    if "02_docs/HANDOFF.md" in files:
        handoff_path = os.path.join(
            os.path.dirname(os.path.abspath(__file__)), "..", "02_docs", "HANDOFF.md"
        )
        try:
        ...
        if line_count > 100:
            reminders.append(
                f"02_docs/HANDOFF.md が{line_count}行に肥大化（目安100行以内）→ "
                "「現在地」のみ残し、過去ログ・詳細は02_docs/HANDOFF_archive.mdへ移動"
                "（過去の変更内容自体はgit履歴に残るため重複保持不要）"
            )
